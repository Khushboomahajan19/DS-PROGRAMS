...
void del_after(Node **start, int val){
    ...
    if(current != NULL){ //value found
        ...
        if(current->next == NULL){
            printf("\nNode not avalible after %d, deletion not possible",val);
        }else{
...
void del_pos(Node**start,int pos){
    ...
    if(totalNode<pos){
        printf("Invalid position %d to delete a node",pos);
        return;
    }
    if(pos==1){
        del_beg(start);
        ...
        printf("Node at %d position with %d value deleted\n",pos,current->val);
        free(current);
    }
}
void delete_bef(Node**start,int val){
    Node*current=search(*start,val);
    if(current!=NULL){
        if(current==start){
            printf("\nselection not possible before start node");
         ...
         prev->next=current->next;
         printf("%d node deleted",current->next);   
        }
    }
}
// show all the nodes
